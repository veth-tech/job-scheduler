...
const handleJobFinish = async (job) => {
    ...
    const handleUpsert = async (upsertObj) => {
        ...
        if(error){
            new Error("error upserting")
        } 
        return data
    }

    if(!isRecurring){
        response= await handleUpsert({ id: id, nextExecutionTime: null, isActive: false })
    } else {
        if(frequency === "daily"){
            response= await handleUpsert({ id: id, nextExecutionTime: dayjs().add(1, 'day').unix() })
        } else if(frequency === "weekly"){
            response= await handleUpsert({ id: id, nextExecutionTime: dayjs().add(7, 'day').unix() })    
        }
    }
