...
```bash
jobScheduler$ sam build
```

The SAM CLI installs dependencies defined in `job-executer/package.json`, creates a deployment package, and saves it in the `.aws-sam/build` folder.
...
```bash
jobScheduler$ sam local invoke JobExeFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
jobScheduler$ sam local start-api
jobScheduler$ curl http://localhost:3000/
```
...
```bash
jobScheduler$ sam logs -n JobExeFunction --stack-name jobScheduler --tail
```
...
```bash
aws cloudformation delete-stack --stack-name jobScheduler
```
